                ...
                if ($city) {
                    if ($metro) {
                        ...
                        if ($unitedCities) {
                            ...
                            $hotels = $hotels->whereHas('address', static function (Builder $q) use ($unitedCities) {
            ...
            if ($city) {
                if ($metro) {
                    ...
                    if ($unitedCities) {
                        $unitedHotelsBool = true;
                        $unitedCities = $unitedCities->unitedCities();
                        $rooms = $rooms->whereHas('hotel', function (Builder $q_hotel) use ($unitedCities) {
                            $q_hotel->whereHas('address', function ($q) use ($unitedCities) {
                                foreach ($unitedCities as $key => $unitedCity) {
                                    if ($key === 0) {
                                        $q->where('city', $unitedCity);
                                    } else {
