<template>
	...
	<div class="container mx-auto">
		...
		<div class="mb-4 flex flex-col lg:flex-row">					
			<div v-html="model.page.content" class="flex flex-col lg:flex-row lg:w-1/2 text-sm leading-4 mb-8"></div>
					<div class="flex lg:pl-12 justify-center lg:w-1/2">
						<div class="flex flex-col lg:flex-row items-center mr-6">
							<div class="text-blue-400  xl:text-6xl text-5xl font-semibold mr-2">{{$page.props.count.cities}}</div>
							<div>городов</div>
						</div>
						<div class="flex flex-col lg:flex-row items-center mr-6">
							<div class="text-blue-400 xl:text-6xl text-5xl font-semibold mr-2">{{$page.props.count.hotels}}</div>
							<div>отелей</div>
						</div>
						<div class="flex flex-col lg:flex-row items-center">
							<div class="text-blue-400 xl:text-6xl text-5xl font-semibold mr-2">{{$page.props.count.rooms}}</div>
							<div>номеров</div>
						</div>
					</div>
				</div>
			...
			>
				<div class="w-full bg-white rounded-3xl p-4">
					...
					<div class="text-sm leading-4">
						<a :href="'tel:' + formatPhoneLink($page.props.contacts.phone2)" class="flex items-center mb-2">
							<svg width="20" height="16" viewBox="0 0 20 16" fill="none" xmlns="http://www.w3.org/2000/svg">
								<path d="M15.9996 12.7909V10.9845C16.007 10.6891 15.9053 10.4014 15.7139 10.176C15.1743 9.5404 13.3236 9.13387 12.59 9.29C12.0182 9.41173 11.6181 9.98587 11.227 10.3763C9.71642 9.519 8.46572 8.2708 7.6068 6.7632C7.99793 6.37286 8.57325 5.97357 8.69522 5.40285C8.85142 4.67185 8.44542 2.83299 7.81421 2.29123C7.59219 2.10067 7.30831 1.99718 7.0155 2.00006H5.2054C4.51572 2.00071 3.94192 2.6193 4.00471 3.31279C4 9.29 8.66669 14 14.6843 13.9953C15.3815 14.0582 16.0024 13.4815 15.9996 12.7909Z" stroke="#6170FF" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
							</svg>
							<span class="pl-2">
								<b>
					...
					<Button @click="toggleMapVisibility()" v-bind:classes="isMapVisible ? 'mb-4 w-full lg:w-3/12' : 'mt-3 w-full'">
						{{ showMapBtnText }}
					</Button>
