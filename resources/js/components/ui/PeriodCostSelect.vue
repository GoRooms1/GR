<template>
  <div class="grid gap-[16px]">
    <filter-select :options-array="options" not-null v-model="type" />
    <button
      v-for="range in ranges"
      :data-key="range.key"
      :data-type="range.type_key"
      ...
      :class="
        selectedOption == range.type_key + '_' + range.key
...
export default {
  ...
  mounted() {
    if (this.selectedOption) this.type = this.selectedOption.split("_")[0];
    else this.type = this.options[0].key;
  },
  data() {
    return {
      type: null,
  ...
  computed: {
    ranges() {
      ...
      this.options.forEach((type) => {
        ...
        if (this.type && type_key != this.type) return;
