<template>
  <div v-click-outside="hide" class="relative z-[5] mt-2">
      ...
      <div class="absolute top-[2rem] left-0 z-10 w-full" :class="isActive ? 'block' : 'hidden'">
        ...
        <div ref="datepicker" class="rounded-t-lg p-4 flex items-center justify-center bg-white w-[24.25rem] max-w-[calc(100vw-6.25rem)] leading-4">
          <input ref="day" @input="changeDay" class="w-[2.25rem] py-2 px-2.5 text-[#A7ABB7] text-sm bg-[#EAEFFD] rounded-lg mx-2" value="16" />
          .
          <input ref="month" @input="changeMonth" class="w-[2.25rem] py-2 px-2.5 text-[#A7ABB7] text-sm bg-[#EAEFFD] rounded-lg mx-2" value="11" />
          .
          <input ref="year" @input="changeYear" class="w-[3.25rem] py-2 px-2.5 text-[#A7ABB7] text-sm bg-[#EAEFFD] rounded-lg mx-2" value="2022" />
        </div>
      </div>
    </div>
</template>
