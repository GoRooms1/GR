                                    ...
                                    <ul class="advanced-search-prices-list">
                                        @foreach ($costTypes as $type)
                                            <li class="advanced-search-prices-item">
                                                <input id="advanced-search-prices-{{ $loop->iteration }}"
                                                       type="checkbox" class="checkbox" name="search-price"
                                                       value="{{ $type->slug }}" @checked($type->slug, $search_price)>
                                                <label for="advanced-search-prices-{{ $loop->iteration }}"
                                                       class="checkbox-label checkbox-label-orange">{{ $type->name }}</label>
                                            </li>
                                        @endforeach
                                    </ul>
                                </div>
                                        ...
                                        <ul class="advanced-search-prices-list">
                                            @foreach($type->filterCosts as $cost)
                                                ...
                                                @if ($flag)
                                                    ...
                                                    @endphp
                                                    <li class="advanced-search-prices-item">
                                                        <input id="advanced-search-prices-{{ $cost->id  . '00' }}-{{ $loop->parent->iteration }}"
                                                               name="cost" type="radio"
                                                               class="checkbox"
                                                               value="{{ $value2 }}" @checked($value2, $costRequest) >
                                                ...
                                                @endif
                                                <li class="advanced-search-prices-item">
                                                    <input id="advanced-search-prices-{{ $cost->id }}-{{ $loop->parent->iteration }}"
                                                           name="cost" type="radio"
                                                           class="checkbox"
                                                           value="{{$value}}" @checked($value, $costRequest) >
                ...
                <div class="left">
                    <div class="advanced-search-filter-item">
                        <p for="advanced-search-filter-profitably"
                           class="search-filter-label search-filter-label-profitably">
                            Выгодно</p>
                    </div>
            ...
            <form action="{{ route('search') }}" autocomplete="off" id="search-form" class="search-form" method="GET">
              ...
              @if(Route::currentRouteNamed('custom.*'))
                  ...
                  <i class="fa-solid fa-xmark"></i>
                            ...
                            @else
                                <a href="{{ \App\Widgets\Filter::remove_attr($attr->model === \Domain\Room\Models\Room::class ? 'room' : 'hotel', $attr->id) }}">
                  <i class="fa-solid fa-xmark"></i>
                </a>
                            @endif
            </span>
