...
@section('content')
  ...
  @foreach($users as $user)
    <div class="popup" data-id="{{ $user->id }}">
      ...
      <form action="{{ route('moderator.staff.update', $user->id) }}" method="post">
        @csrf
        @method('PUT')
        <input type="text"
               class="field"
      ...
      <form action="{{ route('moderator.staff.remove', $user->id) }}" method="POST" class="remove-user d-none">
        @method('DELETE')
        @csrf
      </form>

      <form action="{{ route('moderator.staff.update.password', $user->id) }}" method="post" class="generate-password-user d-none">
        @csrf
      </form>
    </div>
  @endforeach
