                                        ...
                                        @foreach ($hotel->metros as $metro)
                                            <li class="metro">
                                                <a href="/address/{{ Str::slug($hotel->address->city) }}/metro-{{ Str::slug($metro->name) }}">
                                                    ...
                                                    {{ $metro->name }} - {{ $metro->distance }} мин <img
                                                            class="svg-walk" src="{{asset('img/walk.svg')}}" alt="">
                                                </a>
                                            </li>
                                        @endforeach
                                    ...
                                    @foreach ($hotel->images as $image)
                                        @if ($image->moderate)
                                            <div class="swiper-slide product-slide-small"
                                                 data-full="{{ asset('img/hotel-moderate.jpg') }}">
                                                <img itemprop="photo" class="swiper-lazy"
                    ...
                    @if ($hotel->metros()->count())
                        @foreach ($hotel->metros as $metro)
                            <li class="metro">
                                <a href="/address/{{ Str::slug($hotel->address->city) }}/metro-{{ Str::slug($metro->name) }}">
                                    <i class="icon-metro mr-2" style="color: #{{ $metro->color }}"></i>
                                    {{ $metro->name }} - {{ $metro->distance }} мин <img class="svg-walk"
                ...
                <div id="rooms" class="tab-pane fade in active">
                    ...
                    <div class="show-more">
                        <p class="show-more-counter">
                            Загружено: {{ \Domain\Room\Models\Room::PER_PAGE < $hotel->rooms()->count() ? \Domain\Room\Models\Room::PER_PAGE : $hotel->rooms()->count() }}
                            ({{ $hotel->rooms()->count() }})</p>
                        @if ($hotel->rooms()->count() > \Domain\Room\Models\Room::PER_PAGE)
