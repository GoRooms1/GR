        ...
        <div class="container">
            <ul itemscope itemtype="https://schema.org/BreadcrumbList" >
                 ...
                 <li  itemid="http://schema.org/breadcrumb" itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem">
                        <a id="breadcrumbs" itemscope  itemtype="http://schema.org/Thing" itemprop="item" href="/{{$item['url']}}"><span  itemprop="name">{{$item['text']}}</span></a>
            ...
            <div class="row">
                                ...
                                <ul class="room-metro">
                                    @if ($hotel->metros()->count())
                                        @foreach ($hotel->metros as $metro)
                                            <li class="metro"><a
                                                    href="/address/{{ \Str::slug($hotel->address->city) }}/metro-{{ \Str::slug($metro->name) }}"><img
                                                        src="{{ asset('/img/ico-metro-'.$metro->color.'.svg') }}"
                                                        alt="">{{ $metro->name }} - {{ $metro->distance }}</a></li>
                                        @endforeach
                ...
                <div class="col-lg-7 col-xxl-6 block-desktop">
                    <div class="product-slider-wrapper hotel-product-slider-wrapper">
                                ...
                                @foreach ($hotel->images as $image)
                                    <a href="{{ asset($image->path) }}" data-fancybox="gallery"
                                       class="swiper-slide product-slide-big">
                                        <img itemprop="photo" class="swiper-lazy" data-src="{{ asset($image->path) }}?w=640&h=300&fit=crop&fm=webp" src="{{ asset('img/pr600x300.jpg') }}" alt="">
                                    </a>
                                @endforeach
                            </div>
                        </div>
                                ...
                                <div class="swiper-wrapper">
                                    @foreach ($hotel->images as $image)
                                        <div class="swiper-slide product-slide-small">
                                            <img itemprop="photo" class="swiper-lazy" data-src="{{ asset($image->path) }}?w=125&h=85&fit=crop&fm=webp" src="{{ asset('img/pr125x85.jpg') }}" alt="">
                                        </div>
                                    @endforeach
                                </div>
                            </div>
                        ...
                        <div class="rating">
                            <p class="rating-title" itemprop="ratingValue">Рейтинг <span>{{ round($hotel->ratings()->avg('value'), 1) }}</span>
    ...
    <section class="section block-mobile">
        <div class="container">
            ...
            <div class="h2 section-title">
                <a class="room-card-more js-details-list-btn" href="#details-list-wrapper-m" role="button" data-toggle="collapse" aria-expanded="false"
                        ...
                        <div class="swiper-wrapper">
                            @foreach ($hotel->images as $image)
                                <a href="{{ asset($image->path) }}" data-fancybox="gallery-m"
                                   class="swiper-slide product-slide-big">
                                    <img class="swiper-lazy" data-src="{{ asset($image->path) }}?w=640&h=300&fit=crop&fm=webp" src="{{ asset('img/pr600x300.jpg') }}" alt="">
                                </a>
                            @endforeach
                        </div>
                    </div>
